...
std::wstring GetAssemblyName(
  ...
  hr = assembly_import->GetAssemblyProps(
      current, nullptr, nullptr, nullptr, (WCHAR*)name.data(),
...
ModuleInfo GetModuleInfo(ICorProfilerInfo3* info, const ModuleID& module_id) {
  ...
  const HRESULT hr = info->GetModuleInfo2(
      ...
      &module_path_len, (WCHAR*)module_path.data(), &assembly_id,
      &module_flags);
  ...
  switch (token_type) {
    ...
    case mdtTypeRef:
      hr = metadata_import->GetTypeRefProps(
          token, &parent_token, (WCHAR*)type_name.data(),
    ...
    case mdtTypeSpec:
      ...
      break;
    case mdtModuleRef:
      metadata_import->GetModuleRefProps(token, (WCHAR*)type_name.data(),
                                         (DWORD)(type_name.size()),
                                         &type_name_len);
      break;
    case mdtMemberRef:
